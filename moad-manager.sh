...
generate_env_file() {
    ...
    # Generate MySQL Grafana password (Grafana uses default "admin" password - change after first login)
    local mysql_grafana_password
    
    if [ -n "$existing_mysql_grafana_password" ]; then
        dialog --stdout --yesno "Regenerate MySQL Grafana password?" 8 50 2>&1 >/dev/null
        if [ $? -eq 0 ]; then
            dialog --stdout --infobox "Generating new secure random password..." 5 50 2>&1 >/dev/null
            sleep 1
            mysql_grafana_password=$(generate_password)
        else
            mysql_grafana_password="$existing_mysql_grafana_password"
        fi
    else
        dialog --stdout --infobox "Generating secure random password..." 5 50 2>&1 >/dev/null
        sleep 1
        mysql_grafana_password=$(generate_password)
    fi
...
EOF
    ...
    success_msg+="Generated Passwords:\n"
    ...
    success_msg+="Grafana Access:\n"
    success_msg+="  URL: ${GRAFANA_URL}\n"
    success_msg+="  Username: admin\n"
    success_msg+="  Password: admin (default - change after first login)"
...
show_service_urls() {
    ...
    info+="  Username: admin\n"
    info+="  Password: admin (default - change after first login)\n"
    info+="  Note: If already initialized, password may differ\n"
    info+="\n"
