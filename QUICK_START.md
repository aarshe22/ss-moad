...
- Passwords are randomly generated by `generate-env.sh` for security
...
```bash
./moad-manager.sh
# Select "4. Docker: Start All Containers" from the menu
```
...
- **Grafana**: http://dev1.schoolsoft.net:3000
  - Username: `admin`
  - Password: (from `.env` file - displayed when running `generate-env.sh`)
